    ...
    static TypeId tid =
        TypeId("ns3::aodv::RoutingProtocol")
            ...
            .AddAttribute("EnableWrmAttack",
                          "Indicates whether a Wormhole Attack enable.",
                          BooleanValue(false),
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    /*
      When a malicious node receives an AODV control packet with the sender's IP address as the
      first end of the P2P interface wormhole tunnel and the receiver's address as the second end of
      the WiFi interface wormhole tunnel, then explicitly set the receiver's IP address as the
      second end P2P interface of the wormhole tunnel.
    */
    if (EnableWrmAttack)
    {
        ...
        /*
          When a malicious wormhole node receives an AODV control packet with the sender's IP
          address as the first end of the WiFi interface wormhole tunnel and receiver's address as
          the second end of the P2P interface wormhole tunnel, then explicitly set the receiver's IP
          address as the first end P2P interface of the wormhole tunnel
        */
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (!m_routingTable.LookupRoute(origin, toOrigin))
    {
        ...
        /* When the first wormhole node receives a packet and the source is the P2P interface,
        get the net device of the second P2P interface and set the receiver to be the second P2P IP
        address. As the PointToPoint net device does not notify active, it is not considered in the
        reverse route.
        */
        ...
        /* When the second wormhole node receives a packet and the source is the P2P interface,
        get the net device of the first P2P interface and set the receiver to be the first P2P IP
        address. As the PointToPoint net device does not notify active, it is not considered in the
        reverse route.
        */
...
RoutingProtocol::ProcessHello(const RrepHeader& rrepHeader, Ipv4Address receiver)
{
   ...
   If the second wormhole node receives an RREP packet from the first wormhole node,
   then we have set the output device to the WiFi interface net device and also set the interface of
   the second P2P interface IP address.
*/
        if (EnableWrmAttack && wrmDst == FirstP2PEndOfWormTunnel)
        {
            toNeighbor.SetOutputDevice(
                m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(SecondP2PEndOfWormTunnel)));
            toNeighbor.SetInterface(
                m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(SecondP2PEndOfWormTunnel), 0));
        }
        /* Added lines for the false entry in the routing table successfully added.
           If the first wormhole node receives an RREP packet from the second wormhole node,
           then we have set the output device to the WiFi interface net device and also set the
           interface of the first P2P interface IP address.
        */
        else if (EnableWrmAttack && wrmDst == SecondP2PEndOfWormTunnel)
        {
            toNeighbor.SetOutputDevice(
                m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(FirstP2PEndOfWormTunnel)));
            toNeighbor.SetInterface(
                m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(FirstP2PEndOfWormTunnel), 0));
        }
        else
        {
            toNeighbor.SetOutputDevice(
